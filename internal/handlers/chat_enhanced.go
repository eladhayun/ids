...
// @Router /api/chat/enhanced [post]
func ChatEnhancedHandler(db *sqlx.DB, cfg *config.Config, cache *cache.Cache, embeddingService *embeddings.EmbeddingService, writeClient *database.WriteClient) echo.HandlerFunc {
	// Create email embedding service
	emailService, err := emails.NewEmailEmbeddingService(cfg, writeClient)
	if err != nil {
		fmt.Printf("[CHAT_ENHANCED] Warning: Failed to create email service: %v\n", err)
		emailService = nil // Will skip email search if not available
	}

	return func(c echo.Context) error {
		fmt.Printf("[CHAT_ENHANCED] ===== NEW ENHANCED CHAT REQUEST =====\n")

		// Handle case where database connection is not available
		if db == nil {
			fmt.Printf("[CHAT_ENHANCED] ERROR: Database connection not available\n")
		...
		if cfg.OpenAIKey == "" {
			fmt.Printf("[CHAT_ENHANCED] ERROR: OpenAI API key not configured\n")
		...
		if err := c.Bind(&req); err != nil {
			fmt.Printf("[CHAT_ENHANCED] ERROR: Invalid request body: %v\n", err)
			return c.JSON(http.StatusBadRequest, models.ChatResponse{
				Error: fmt.Sprintf("Invalid request body: %v", err),
			})
		}

		fmt.Printf("[CHAT_ENHANCED] Received conversation with %d messages\n", len(req.Conversation))

		// Validate conversation is not empty
		if len(req.Conversation) == 0 {
			fmt.Printf("[CHAT_ENHANCED] ERROR: Empty conversation\n")
			return c.JSON(http.StatusBadRequest, models.ChatResponse{
		...
		if userQuery == "" {
			fmt.Printf("[CHAT_ENHANCED] ERROR: No user message found in conversation\n")
			return c.JSON(http.StatusBadRequest, models.ChatResponse{
				Error: "No user message found in conversation",
			})
		}

		fmt.Printf("[CHAT_ENHANCED] Extracted user query: '%s'\n", userQuery)

		// Check for shipping inquiry
		if isShipping, country := IsShippingInquiry(userQuery); isShipping {
			fmt.Printf("[CHAT_ENHANCED] Detected shipping inquiry for country: %s\n", country)
		...
		messages := buildEnhancedOpenAIMessages(
			req.Conversation,
			inStockProducts,
			similarEmails,
		...
		if err != nil {
			fmt.Printf("[CHAT_ENHANCED] ERROR: OpenAI API error: %v\n", err)
			return c.JSON(http.StatusInternalServerError, models.ChatResponse{
		...
		if len(resp.Choices) == 0 {
			fmt.Printf("[CHAT_ENHANCED] ERROR: No response from OpenAI\n")
			return c.JSON(http.StatusInternalServerError, models.ChatResponse{
...
// buildEnhancedOpenAIMessages creates OpenAI messages with product and email context
func buildEnhancedOpenAIMessages(
